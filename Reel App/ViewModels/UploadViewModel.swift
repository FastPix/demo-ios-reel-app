...
final class UploadViewModel: NSObject,ObservableObject, UploadSDKErrorDelegate  {
    ...
    func uploadVideo(
        ...
        do {
            ...
            guard let (signedUrl, uploadId) = try await uploadService.createDirectUpload(creator_id: createrID, creator_name: createrName, title: title, description: description) else {
                return
            }
